/** Omexa QR v2.1.0 - With Loading Animation - MIT License */
'use strict';
...
function QRCode(r) {
  ...
  function addStyles() {
    if (!document.getElementById("omexa-qr-styles")) {
      const style = document.createElement("style");
      style.id = "omexa-qr-styles";
      style.textContent = "@keyframes omexa-spin{0%{transform:rotate(0deg)}100%{transform:rotate(360deg)}}@keyframes omexa-pulse{0%,100%{opacity:0.4}50%{opacity:0.8}}@keyframes omexa-grid{0%{opacity:0.1;transform:translateX(0)}50%{opacity:0.3}100%{opacity:0.1;transform:translateX(10px)}}";
      document.head.appendChild(style)
    }
  }

  function createLoadingAnimation(size, uniqueId) {
    const finalSize = Math.max(size || 128, 128);
    return `<div id="omexa-loader-${uniqueId}" style="width:${finalSize}px;height:${finalSize}px;display:flex;flex-direction:column;align-items:center;justify-content:center;background:linear-gradient(135deg,#f8f9fa 0%,#e9ecef 100%);border:2px dashed #6c757d;border-radius:8px;margin:10px 0;position:relative;overflow:hidden;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;"><div style="position:absolute;top:0;left:0;right:0;bottom:0;background-image:linear-gradient(90deg,rgba(0,102,204,0.1) 1px,transparent 1px),linear-gradient(rgba(0,102,204,0.1) 1px,transparent 1px);background-size:12px 12px;animation:omexa-grid 3s linear infinite;"></div><div style="width:32px;height:32px;border:3px solid #e9ecef;border-top:3px solid #0066cc;border-radius:50%;animation:omexa-spin 1s linear infinite;margin-bottom:12px;position:relative;z-index:2;"></div><div style="font-size:12px;color:#6c757d;font-weight:500;z-index:2;animation:omexa-pulse 2s ease-in-out infinite;">Generating QR...</div></div>`
  }
  ...
  function generateQR(text, size) {
    ...
    try {
      ...
      return svgElement.outerHTML ? svgElement.outerHTML : svgElement
    } catch (error) {
  ...
  function processQRTags() {
    addStyles();
    const qrTags = document.querySelectorAll("qr");
    console.log(`🔍 Found ${qrTags.length} QR tags to process`);
    qrTags.forEach((tag, index) => {
      const link = tag.getAttribute("link");
      const size = parseInt(tag.getAttribute("size")) || 128;
      const uniqueId = Date.now() + "-" + index;
      console.log(`📱 Processing QR tag ${index+1}: "${link}" (size: ${size})`);
      if (link) {
        tag.outerHTML = createLoadingAnimation(size, uniqueId);
        setTimeout(() => {
          try {
            const qrSvg = generateQR(link, size);
            const loader = document.getElementById(`omexa-loader-${uniqueId}`);
            if (loader) {
              const styledSvg = qrSvg.replace("<svg", '<svg style="display: block; margin: 10px 0; opacity: 0; transition: opacity 0.5s ease-in-out;"');
              loader.outerHTML = styledSvg;
              setTimeout(() => {
                const newSvg = document.querySelector('svg[style*="opacity: 0"]');
                if (newSvg) {
                  newSvg.style.opacity = "1"
                }
              }, 100)
            }
            console.log(`✅ QR tag ${index+1} processed successfully!`)
          } catch (error) {
            console.error(`❌ Error generating QR code ${index+1}:`, error);
            const loader = document.getElementById(`omexa-loader-${uniqueId}`);
            if (loader) {
              loader.outerHTML = `<div style="color:#dc3545;border:2px solid #dc3545;padding:16px;margin:10px 0;background:#f8d7da;border-radius:6px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;text-align:center;font-size:14px;"><strong>❌ QR Error</strong><br><small>${error.message}</small></div>`
            }
          }
        }, 400 + index * 150)
      } else {
        console.error(`❌ QR tag ${index+1} missing link attribute`);
        tag.outerHTML = '<div style="color:#dc3545;border:2px solid #dc3545;padding:16px;margin:10px 0;background:#f8d7da;border-radius:6px;font-family:-apple-system,BlinkMacSystemFont,\'Segoe UI\',Roboto,sans-serif;text-align:center;font-size:14px;"><strong>❌ Error</strong><br><small>Missing link attribute</small></div>'
      }
    });
    console.log("🎉 All QR tags processing initiated!")
  }
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", processQRTags)
  } else {
    processQRTags()
  }
  window.OmexaQR = {
    generate: generateQR,
    process: processQRTags,
    createLoader: createLoadingAnimation,
    version: "2.1.0"
  }
})();
